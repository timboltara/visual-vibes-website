...
const defaultFilters: Filters = { categories: [], sizes: [], priceRange: null, sort: "featured" };

// Women's base: everything that isn't men's-only
const womensBase = products.filter((p) => p.gender !== "mens");

function applyFilters(all: Product[], f: Filters): Product[] {
  let list = [...all];
  if (f.categories.length > 0) list = list.filter((p) => f.categories.includes(p.category));
  if (f.sizes.length > 0) list = list.filter((p) => f.sizes.some((s) => p.sizes.includes(s)));
  if (f.priceRange) {
...
function FilterSection({ title, children, defaultOpen = true }: { title: string; children: React.ReactNode; defaultOpen?: boolean }) {
  ...
  return (
    <div className="border-b border-gray-100">
      <button onClick={() => setOpen((o) => !o)} className="flex items-center justify-between w-full py-3 font-heading text-[10px] uppercase tracking-widest text-vv-black font-semibold">
        {title}
...
export default function ForHerPage() {
  ...
  const results = applyFilters(womensBase, filters);
  ...
  const toggleCategory = useCallback((val: string) => {
    setFilters((f) => ({ ...f, categories: f.categories.includes(val) ? f.categories.filter((c) => c !== val) : [...f.categories, val] }));
  }, []);

  const toggleSize = useCallback((val: string) => {
    setFilters((f) => ({ ...f, sizes: f.sizes.includes(val) ? f.sizes.filter((s) => s !== val) : [...f.sizes, val] }));
  }, []);
    ...
    <div className="flex flex-col gap-0">
      <FilterSection title="Sort By">
        <div className="flex flex-col gap-1">
          {([{ label: "Featured", value: "featured" }, { label: "Price: Low to High", value: "price-asc" }, { label: "Price: High to Low", value: "price-desc" }, { label: "Newest", value: "newest" }] as { label: string; value: SortOption }[]).map((opt) => (
            <label key={opt.value} className="flex items-center gap-2.5 cursor-pointer py-1" onClick={() => setFilters((f) => ({ ...f, sort: opt.value }))}>
              <div className={`w-3.5 h-3.5 rounded-full border flex-shrink-0 flex items-center justify-center ${filters.sort === opt.value ? "border-vv-black" : "border-gray-300"}`}>
      ...
      <FilterSection title="Category">
        <div className="flex flex-col gap-1">
          {CATEGORIES.map((c) => (
            <label key={c.value} className="flex items-center gap-2.5 cursor-pointer py-1" onClick={() => toggleCategory(c.value)}>
              <div className={`w-3.5 h-3.5 border flex-shrink-0 flex items-center justify-center ${filters.categories.includes(c.value) ? "border-vv-black bg-vv-black" : "border-gray-300"}`}>
                {filters.categories.includes(c.value) && <span className="text-white font-bold" style={{ fontSize: "9px" }}>✓</span>}
              </div>
      ...
      <FilterSection title="Size">
        <div className="flex flex-wrap gap-1.5">
          {ALL_SIZES.map((s) => (
            <button key={s} onClick={() => toggleSize(s)} className={`font-heading text-[10px] uppercase tracking-wide px-3 py-2 border transition-all ${filters.sizes.includes(s) ? "border-vv-black bg-vv-black text-white" : "border-gray-200 text-vv-black hover:border-vv-black"}`}>{s}</button>
          ))}
        </div>
      </FilterSection>
      <FilterSection title="Price" defaultOpen={false}>
        <div className="flex flex-col gap-1">
          {PRICE_RANGES.map((r) => (
            <label key={r.label} className="flex items-center gap-2.5 cursor-pointer py-1" onClick={() => setFilters((f) => ({ ...f, priceRange: f.priceRange === r.label ? null : r.label }))}>
              <div className={`w-3.5 h-3.5 rounded-full border flex-shrink-0 flex items-center justify-center ${filters.priceRange === r.label ? "border-vv-black" : "border-gray-300"}`}>
  ...
  return (
    <div className="pt-[129px] min-h-screen bg-white">
      <div className="px-5 sm:px-10 pt-10 pb-4 max-w-screen-xl mx-auto flex flex-col sm:flex-row sm:items-end justify-between gap-2">
        <div>
          <p className="font-heading text-[10px] uppercase tracking-widest text-vv-orange mb-1">Shop Women</p>
          <h1 className="font-heading font-black text-4xl sm:text-5xl uppercase text-vv-black leading-none">For Her</h1>
        </div>
      ...
      <div className="lg:hidden border-b border-gray-100 px-5 py-3 flex items-center justify-between sticky top-[129px] z-40 bg-white">
        <button onClick={() => setMobileFiltersOpen(true)} className="flex items-center gap-2 font-heading text-xs uppercase tracking-widest text-vv-black border border-gray-200 px-4 py-2.5 hover:border-vv-black transition-colors">
          <FiSliders size={13} /> Filters
          {activeCount > 0 && <span className="w-4 h-4 bg-vv-orange text-white font-heading text-[9px] font-bold rounded-full flex items-center justify-center">{activeCount}</span>}
        </button>
        {activeCount > 0 && <button onClick={clearAll} className="font-heading text-[10px] uppercase tracking-widest text-vv-gray-mid hover:text-vv-black transition-colors">Clear all</button>}
      </div>
      ...
      <div className="max-w-screen-xl mx-auto px-5 sm:px-10 py-6 flex gap-8 items-start">
        <aside className="hidden lg:flex lg:flex-col w-56 flex-shrink-0 sticky top-[145px] max-h-[calc(100vh-170px)]">
          <div className="flex items-center justify-between mb-4 flex-shrink-0">
            ...
            {activeCount > 0 && <button onClick={clearAll} className="font-heading text-[9px] uppercase tracking-widest text-vv-gray-mid hover:text-vv-black transition-colors">Clear all</button>}
          </div>
          <div className="overflow-y-auto flex-1 pr-1">{SidebarContent}</div>
        </aside>
        ...
        <div className="flex-1 min-w-0">
          {results.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-20 gap-4 text-center">
              ...
              <button onClick={clearAll} className="font-heading text-xs uppercase tracking-widest text-vv-orange hover:underline">Clear all filters</button>
            </div>
          ) : (
      ...
      <AnimatePresence>
        {mobileFiltersOpen && (
          <>
            <motion.div key="backdrop" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/50 z-50 lg:hidden" onClick={() => setMobileFiltersOpen(false)} />
            <motion.div key="drawer" initial={{ x: "-100%" }} animate={{ x: 0 }} exit={{ x: "-100%" }} transition={{ type: "spring", damping: 30, stiffness: 300 }} className="fixed top-0 left-0 bottom-0 w-72 bg-white z-50 flex flex-col shadow-2xl lg:hidden">
              <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
                <p className="font-heading text-xs uppercase tracking-widest text-vv-black font-semibold">Filters</p>
                <button onClick={() => setMobileFiltersOpen(false)}><FiX size={18} className="text-vv-black" /></button>
              </div>
              <div className="flex-1 overflow-y-auto px-5 py-4">{SidebarContent}</div>
              <div className="px-5 py-4 border-t border-gray-100">
                <button onClick={() => setMobileFiltersOpen(false)} className="w-full bg-vv-black text-white font-heading text-xs font-bold uppercase tracking-widest py-4 hover:bg-vv-orange transition-colors">
...
function ProductCard({ product, onAddToCart }: { product: Product; onAddToCart: (p: Product, size: string) => void }) {
  ...
  return (
    <motion.div layout initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.96 }} transition={{ duration: 0.25 }} className="group">
